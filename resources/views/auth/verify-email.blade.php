            ...
            <div class="bg-white rounded-lg w-96 sm:rounded-xl shadow-xl max-w-xs sm:max-w-sm md:max-w-md p-4 sm:p-6 md:p-8">
                <div class="text-center mb-4 sm:mb-6">
                    ...
                    <span class="text-xs sm:text-sm md:text-base text-gray-600 leading-relaxed">
                        We've sent a 6-digit OTP code to <strong>{{ $user->email }}</strong>
                    </span>
                </div>
                ...
                <form method="POST" action="{{ route('email.verify') }}" class="space-y-3 sm:space-y-4">
                    @csrf
                    <div>
                        <label for="otp" class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Enter OTP Code</label>
