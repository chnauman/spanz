...
@section('title', 'Subscription Requests - SPANZ')
@section('content')
    ...
    <div class="w-full">
        <div class="border border-gray-300 p-3 sm:p-4 lg:p-6 bg-white rounded-lg shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between rounded-sm bg-[#092C48] text-white p-4 mt-6 sm:mt-8 lg:mt-10 space-y-2 sm:space-y-0">
                <h1 class="text-xl sm:text-2xl font-bold">Subscription Requests</h1>
            ...
            <div class="mt-6">
                                    ...
                                    <div class="mt-3">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <span class="text-sm text-gray-500">
                                                by {{ $request->processedBy->name ?? 'System' }} on {{ $request->processed_at ? $request->processed_at->format('M d, Y H:i') : 'N/A' }}
                                            </span>
                                        </div>
                ...
                @else
                    <div class="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                        ...
                        <h3 class="mt-3 text-sm font-medium text-gray-900">No subscription requests</h3>
                        <p class="mt-1 text-xs text-gray-500">No requests found for the selected filter.</p>
                        <div class="mt-4">
                            <a href="{{ route('admin.subscriptions.index') }}" class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
...
function openDeclineModal(requestId) {
    document.getElementById('declineForm').action = `/admin/subscriptions/decline-request/${requestId}`;
