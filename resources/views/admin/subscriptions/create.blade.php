...
@section('title', 'Create Subscription - SPANZ')
@section('content')
    ...
    <div class="w-full">
        <div class="border border-gray-300 p-3 sm:p-4 lg:p-6 bg-white rounded-lg shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between rounded-sm bg-[#092C48] text-white p-4 mt-6 sm:mt-8 lg:mt-10 space-y-2 sm:space-y-0">
                <h1 class="text-xl sm:text-2xl font-bold">Create New Subscription Plan</h1>
            ...
            <form action="{{ route('admin.subscriptions.store') }}" method="POST" class="mt-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" 
                               ...
                               value="{{ old('credits_per_month') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               placeholder="20 (use -1 for unlimited)"
                               required>
                        <p class="text-xs text-gray-500 mt-1">Use -1 for unlimited credits. Credit cost per tender view is managed under <a href="{{ route('admin.tender-view-pricing.index') }}" class="text-blue-600 hover:underline">Tender View Pricing</a>.</p>
                    </div>
